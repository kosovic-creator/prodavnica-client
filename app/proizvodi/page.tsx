...
export default async function ProizvodiPage({ searchParams }: { searchParams: Promise<{ page?: string; pageSize?: string; lang?: string; search?: string }> }) {
  ...
  return (
    <ClientLayout lang={lang} isLoggedIn={isLoggedIn} korisnikIme={korisnikIme}>
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 py-8 text-gray-900 dark:text-gray-900">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {proizvodi.length === 0 ? (
                ...
                proizvodi.map((proizvod: any) => {
                  ...
                  return (
                    <div key={proizvod.id} className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col shadow-sm hover:shadow-md transition-shadow relative text-gray-900 dark:text-gray-900">
                      ...
                      <div className="flex-1 space-y-2">
                        <h3 className="font-semibold text-lg text-gray-900 dark:text-gray-900 line-clamp-2">{naziv}</h3>
                        <p className="text-gray-600 dark:text-gray-700 text-sm line-clamp-2">{opis}</p>
                        <p className="text-gray-500 dark:text-gray-700 text-xs line-clamp-1">{karakteristike}</p>
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-500 dark:text-gray-700">{t['kategorija']}: {kategorija || t['nema_kategorije']}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <div className="text-xl font-bold text-blue-700">{proizvod.cena} €</div>
                          <div className={`text-xs font-medium px-2 py-1 rounded ${proizvod.kolicina === 0 ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-700 dark:text-gray-900'}`}>{t['kolicina']}: {proizvod.kolicina}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 mt-4">
                        <a href={`/proizvodi/${proizvod.id}?lang=${lang}`} className="flex-1 bg-gray-100 text-gray-700 dark:text-gray-900 py-2 px-3 rounded-lg hover:bg-gray-200 transition-colors duration-200 flex items-center justify-center gap-2 text-sm font-medium">
                          {t['detalji']}
                        </a>
